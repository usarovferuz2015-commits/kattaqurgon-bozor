...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <div className="card group animate-fade-in relative flex flex-col h-full bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-md transition-all">
      {/* Image Area */}
      <div className="relative aspect-square bg-gray-100 overflow-hidden">
        ...
        {product.is_premium && (
          <span className="absolute top-2 left-2 mt-8 bg-accent-500 text-white text-xs font-bold px-2 py-1 rounded-lg z-10">
            VIP
          </span>
        )}
        ...
        <button
          ...
          className={`absolute top-2 right-2 w-8 h-8 rounded-full flex items-center justify-center shadow-md transition-all duration-300 z-10 ${
            favorited
        ...
        <button
          ...
          className="absolute bottom-2 right-2 w-9 h-9 bg-white/90 backdrop-blur-md rounded-full shadow-lg flex items-center justify-center hover:bg-primary-50 transition-colors z-10"
        >
      ...
      <div className="p-3 flex-1 flex flex-col justify-between">
        <div>
          <Link to={`/product/${product.slug}`} className="block hover:text-primary-600 transition-colors">
