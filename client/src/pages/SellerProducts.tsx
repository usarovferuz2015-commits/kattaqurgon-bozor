...
export default function SellerProducts() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      if (!seller?.id) return { data: [], total: 0 };
      const res = await productService.getSellerProducts(seller.id);
      return res;
    },
    enabled: !!seller?.id,
  });
      ...
      <div className="container-app py-4 space-y-3">
        {isLoading ? (
          ...
          <div className="text-center py-12">
            ...
            <Link to="/seller/add-product" className="btn-primary mt-4 inline-block">
              Mahsulot qo'shish
            </Link>
          </div>
        ) : (
          products.map((product: any) => (
            <div key={product.id} className="card p-3 flex gap-3">
              ...
              <div className="flex flex-col gap-1">
                <button className="p-2 hover:bg-gray-100 rounded-lg text-dark-400">
                  <FiEdit2 className="w-4 h-4" />
                </button>
                <button
                  onClick={() => handleDelete(product.id, product.name_uz)}
